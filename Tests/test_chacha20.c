...
TEST_GROUP_RUNNER(Chacha20) {
    RUN_TEST_CASE(Chacha20, Chacha20_01);
    RUN_TEST_CASE(Chacha20, Chacha20_02);
    RUN_TEST_CASE(Chacha20, Chacha20_03);
    RUN_TEST_CASE(Chacha20, Chacha20_04);
}

TEST(Chacha20, Chacha20_01) {
    ...
    // Encryption
    chacha20(plaintext, strlen((char*)plaintext), key, nonce, initial_block_counter, result);
    TEST_ASSERT_EQUAL_MEMORY(expected_encrypted_msg, result, sizeof(expected_encrypted_msg));

    // Decryption
    chacha20(expected_encrypted_msg, sizeof(expected_encrypted_msg), key, nonce, initial_block_counter, result);
    TEST_ASSERT_EQUAL_MEMORY(plaintext, result, strlen((char*)plaintext));
}

TEST(Chacha20, Chacha20_02) {

    byte_t key[32] = {0x00U};
    byte_t nonce[12] = {0x00U};
    ...
    // Encryption
    chacha20(plaintext, sizeof(plaintext), key, nonce, initial_block_counter, result);
    TEST_ASSERT_EQUAL_MEMORY(expected_encrypted_msg, result, sizeof(expected_encrypted_msg));
